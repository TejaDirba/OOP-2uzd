...
void Student::setNd(const std::vector<int>& g) {
    for (int x: g) if (x<0||x>10) throw std::invalid_argument("ND netinkamas");
    nd = g;
}

void Student::setEgzaminas(int e) {
    if (e<0||e>10) throw std::invalid_argument("Egz netinkamas");
    egzaminas = e;
}

void Student::addHomeworkGrade(int grade) {
    if (grade<0||grade>10) throw std::invalid_argument("Grade netinkamas");
    nd.push_back(grade);
}
