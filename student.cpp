...
Student::Student(): vardas(""), pavarde(""), nd(), egzaminas(0), galutinisVid(0.0), galutinisMed(0.0) {}
Student::Student(const std::string& v, const std::string& p): vardas(v), pavarde(p), nd(), egzaminas(0), galutinisVid(0.0), galutinisMed(0.0) {}
...
void Student::setNd(const std::vector<int>& grades) {
    for (int g: grades) if (g<0||g>10) throw std::invalid_argument("ND pazymys netinkamas");
    nd = grades;
}
void Student::setEgzaminas(int e) {
    if (e<0||e>10) throw std::invalid_argument("Egzamino pazymys netinkamas");
    egzaminas = e;
}
...
void Student::calculateFinalGrades() {
    if (nd.empty()) throw std::logic_error("Nera namu darbu");
...
double calculateAverage(const std::vector<int>& v) {
    if (v.empty()) throw std::invalid_argument("Tuscias vektorius");
...
std::istream& operator>>(std::istream& is, Student& s) {
    std::string v, p;
    if (!(is>>v>>p)) throw std::runtime_error("Klaida skaitant");
    s.setVardas(v);
    s.setPavarde(p);
    return is;
}
